...
export default function NeedDetailPage() {
  ...
  function requireLogin() {
    if (user) return true;
    navigate('/login', { replace: true, state: { from: { pathname: `/need-board/${id}` } } });
    return false;
  }
  ...
  async function handleOffer(event) {
    ...
    if (isSupabaseConfigured) {
      const { error } = await supabase.from('need_offers').insert({ need_post_id: need.id, donor_id: user.id, message, status: 'offered' });
      if (error) {
        setNotice(error.message);
        return;
      }
      await supabase.from('need_posts').update({ status: 'offered' }).eq('id', need.id);
    }
  ...
  return (
    <main className="need-detail-page">
      ...
      <div className="container need-detail-grid">
        ...
        <aside className="offer-panel"><span className="offer-icon"><HandHeart size={24} /></span><span className="eyebrow">Punya barang yang sesuai?</span><h2>Tawarkan bantuanmu</h2><p>Jelaskan barang, kondisinya, serta cara pengambilan yang memungkinkan.</p>{needsLogin && <p className="detail-reassurance">Masuk dulu untuk mengirim tawaran.</p>}{notice && <p className="success-note"><Check size={17} /> {notice}</p>}<form className="form-stack" onSubmit={handleOffer}><label><span>Pesan untuk pembuat kebutuhan</span><textarea required minLength={20} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Saya memiliki barang yang mungkin sesuai..." /></label><button className="btn btn-primary" type="submit">{needsLogin ? 'Masuk untuk tawar' : <>Kirim tawaran <HandHeart size={18} /></>}</button></form><small>Menawarkan barang tidak memerlukan biaya apa pun. Jangan mengirim uang atau data sensitif.</small></aside>
      </div>
    </main>
  );
}
